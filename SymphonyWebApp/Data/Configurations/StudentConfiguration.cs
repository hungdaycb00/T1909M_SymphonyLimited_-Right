...
namespace SymphonyWebApp.Data.Configurations
{
    public class StudentConfiguration : IEntityTypeConfiguration<Student>
    {
        public void Configure(EntityTypeBuilder<Student> builder)
        {
            ...
            builder.Property(x => x.RollNumber).IsRequired().HasMaxLength(10);
            ...
            builder.HasOne(x => x.ClassStudy).WithMany(x => x.Students).HasForeignKey(x => x.RollNumber);
            builder.HasOne(x => x.Course).WithMany(x => x.Students).HasForeignKey(x => x.RollNumber);
        }
    }
}